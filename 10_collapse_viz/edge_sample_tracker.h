...
// .ma_struct file, draw `samples_per_struct` points distributed
// arc-length-proportionally across all the edges that make up that struct's
// curve. For each drawn point, seed one sample on the first gFO face
// incident to that edge. (Seam edges can have 2+ incident faces — one per
// sheet touching them, 3+ at a non-manifold junction — but we no longer
// seed one sample per sheet: joint_lscm_seam_pinned already forces vi/vj/vk
// to a shared UV target across sheets for every seam collapse
// (joint_lscm_pinned.cpp:274-298), so a single sample per point is trusted
// to represent all sheets. This also keeps sample counts manageable, since
// edge_sample_tracker_update()'s per-collapse vertex fixup is O(total
// samples).)
//
...
// samples_per_struct: number of samples drawn per seam/boundary struct_id
//   (each drawn point seeds one sample per incident sheet-face, so a
//   struct's total sample count can exceed samples_per_struct at
//   non-manifold junction edges).
