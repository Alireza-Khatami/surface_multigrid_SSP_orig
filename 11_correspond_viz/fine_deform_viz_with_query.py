...
def _compute_vertex_correspondences():
    """
    Load samples_vertices_*.txt from _bundle_dir.
    Each row: fine_id  coarse_face  b0 b1 b2  bv0 bv1 bv2
    Deformed position = b0*coarseV[bv0] + b1*coarseV[bv1] + b2*coarseV[bv2]
    (fine→coarse direction, exact barycentric eval on coarse mesh geometry).
    """
    global _fine_ids, _fine_pts, _deform_pts, _deform_mesh_v, _fine_id_to_row

    matches = glob.glob(os.path.join(_bundle_dir, "samples_vertices_*.txt"))
    if not matches:
        print(f"[vtx_corr] No samples_vertices_*.txt found in {_bundle_dir}")
        return False

    path = matches[0]
    print(f"[vtx_corr] Loading {os.path.basename(path)}")

    # bv indices in the file are global SSP vertex indices; build inverse map
    # vtxMap[compact_i] = global_i  →  global_to_compact[global_i] = compact_i
    vtxMap = _bundle.vtxMap
    if vtxMap is None:
        print("[vtx_corr] vtxMap missing — cannot map global→compact indices.")
        return False
    global_to_compact = {int(vtxMap[i]): i for i in range(len(vtxMap))}
    ...
    with open(path) as f:
        ...
        for line in lines[1:]:
            ...
            if len(parts) < 8:
                continue
            fine_id = int(parts[0])
            b0, b1, b2    = float(parts[2]), float(parts[3]), float(parts[4])
            gv0, gv1, gv2 = int(parts[5]), int(parts[6]), int(parts[7])
            cv0 = global_to_compact.get(gv0)
            cv1 = global_to_compact.get(gv1)
            cv2 = global_to_compact.get(gv2)
            if cv0 is None or cv1 is None or cv2 is None:
                print(f"[vtx_corr] warn: fine {fine_id} — global vtx not in compact mesh, skipping")
                continue
            pos = b0 * coarseV[cv0] + b1 * coarseV[cv1] + b2 * coarseV[cv2]
            fine_ids_list.append(fine_id)
            deform_pts_list.append(pos)
                    ...
                    if _selected_flipped_face >= 0:
                        ...
                        for k, vid in enumerate(vids):
                            ...
                            if _use_f2c_deform:
                                ...
                                # bundle mode: use deformed position from samples_vertices
                                query_final = deform_pos
                                print(f"  v{k}: fine_id={vid}  (bundle)")
                                print(f"        fine    =({fine_pos[0]:.6f}, {fine_pos[1]:.6f}, {fine_pos[2]:.6f})")
                                print(f"        deformed=({deform_pos[0]:.6f}, {deform_pos[1]:.6f}, {deform_pos[2]:.6f})")
